...
const LotRoute = ({ params }: { params: { id: string } }) => {
    ...
    const clientAction = async (formData: FormData) => {
        ...
        const result = await updateLot(formData, params.id);
    ...
    return (
        <section className='w-[95%] h-[95%] flex flex-col'>
                            ...
                            <form action={clientAction} className='w-full'>
                                <div className='w-3/5 flex flex-col mx-auto gap-4 mt-2'>
                                    <div className='flex justify-around gap-4 w-full'>
                                        ...
                                        <Input name="name" type="text" placeholder="Name of your lot" defaultValue={data?.name || ''} onChange={(e) => setName(e.target.value)} className='flex ml-auto' />
                                    </div>

                                    <div className='flex gap-4 w-full'>
                                        <Label className='flex items-center justify-start w-2/12'>Description</Label>
                                        <Textarea name="description" placeholder="Try to describe your lot, and quality" defaultValue={data?.description || ''} onChange={(e) => setDescription(e.target.value)} className='flex ml-auto' />
                                    </div>

                                    <div className='flex w-full gap-4'>
                                        <Label className='flex items-center justify-start w-2/12'>Exchange</Label>
                                        <Input name="exchangeOffer" type="text" placeholder="Possible exchange offer" defaultValue={data?.exchangeOffer || ''} onChange={(e) => setExchangeOffer(e.target.value)} className='flex ml-auto' />
                                    </div>
                                    <div className='flex w-full gap-4'>
                                        {
                                            !data?.photolot ? (
                                    ...
                                    <div className='flex gap-4 mt-10'>
                                        <SaveButtonBar id={params?.id as string} />
                                    </div>
                                </div>
                            </form>
                        ) : (
            ...
            {
                isLotBelongsToUser && !isEditing && <div className='fixed w-full bottom-0 z-10 bg-[#4A5C6A] border-t border-slate-800 h-24'>
                    <div className='flex items-center justify-between px-5 lg:px-10 h-full'>
                        ...
                        <Button variant="destructive" size='lg' onClick={() => handleDeleteLot(params.id)}>
                            Delete
                        </Button>
                    </div>
                </div>
