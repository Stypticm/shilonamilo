...
const ExchangeStatusSection: React.FC<ExchangeStatusSectionProps> = ({ lot1, lot2, isOwner, fetchData, handleShowFeedback, isShowFeedback, chatId }) => {
    ...
    if (lot1?.Proposal?.[0]?.isOwnerConfirmedExchange && lot2?.Offers?.[0]?.isUserConfirmedExchange ||
        ...
        return (
            <section className='flex justify-around items-center text-center'>
                ...
                <Button variant='outline' size='lg' className='bg-[#b5ad0e]'
        ...
        return !lot1?.Proposal?.[0]?.isOwnerConfirmedExchange || !lot2?.Offers?.[0]?.isOwnerConfirmedExchange ? (
            ...
            <section className='flex justify-around items-center text-center'>
                <span className='text-[#2f6183] font-serif text-3xl'>Waiting for confirmation from partner</span>
            </section>
        );
    } else {
        return lot1?.Offers?.[0]?.isOwnerConfirmedExchange || lot2?.Proposal?.[0]?.isOwnerConfirmedExchange ? (
            ...
            <section className='flex justify-around items-center text-center'>
                <span className='text-[#2f6183] font-serif text-3xl'>Waiting for confirmation from lot owner</span>
            </section>
        );
    }
};
